@extends('layouts.main')
@section('content')
    <div class="container-fluid main-scope-project">
        <div class="row scope-bottom-wrapper">

            <div class="col-xxl-12 recent-xl-77 col-xl-9 box-col-9">
                <div class="row">
                    <div class="col-12">
                        <div class="common-project-header common-space m-b-20">
                            <div class="common-space">
                                <div class="pe-sm-3">
                                    <h5>
                                        Dashboard<span class="badge badge-light-warning ms-2">In Progress</span>
                                    </h5>
                                    <span class="c-o-light">Create a brand logo design for a admin.</span>
                                </div>
                                <div class="common-align">
                                    <div class="customers">
                                        <ul>
                                            <li class="d-inline-block">
                                                <img class="img-40 rounded-circle"
                                                    src="../assets/images/dashboard/user/10.jpg" alt="user" />
                                            </li>
                                            <li class="d-inline-block">
                                                <img class="img-40 rounded-circle"
                                                    src="../assets/images/dashboard/user/11.jpg" alt="user" />
                                            </li>
                                            <li class="d-inline-block">
                                                <div class="common-circle bg-lighter-danger">
                                                    A
                                                </div>
                                            </li>
                                            <li class="d-inline-block">
                                                <img class="img-40 rounded-circle"
                                                    src="../assets/images/dashboard/user/1.jpg" alt="user" />
                                            </li>
                                            <li class="d-inline-block">
                                                <div class="bg-lighter-dark common-circle">
                                                    <span class="f-w-500">9+</span>
                                                </div>
                                            </li>
                                        </ul>
                                    </div>
                                    <h6>All tasks : 24</h6>
                                </div>
                            </div>
                            <div class="common-align">
                                <ul class="common-align">
                                    <li>
                                        <span>Create Date &colon;</span><svg>
                                            <use href="../assets/svg/icon-sprite.svg#vector-calendar"></use>
                                        </svg><span>10 Jul, 2024 </span>
                                    </li>
                                    <li>
                                        <span>Due Date &colon;</span><svg>
                                            <use href="../assets/svg/icon-sprite.svg#vector-calendar"></use>
                                        </svg><span>10 Aug, 2024</span>
                                    </li>
                                </ul>
                                <button class="btn btn-success" type="button" data-bs-toggle="modal"
                                    data-bs-target=".bd-example-modal-lg">
                                    <i class="fa-solid fa-plus"></i> Add Project
                                </button>
                            </div>
                        </div>
                        @include('health.modal_add')
                    </div>
                    <div class="col-12">
                        <div class="tab-content" id="add-product-pills-tabContent">
                            <div class="tab-pane fade show active" id="overview-project" role="tabpanel"
                                aria-labelledby="overview-project-tab">
                                <div class="row">
                                    <div class="col-xxl-12 box-col-12">
                                        <div class="card main-summary">
                                            <div class="card-header card-no-border">
                                                <div class="common-space">
                                                    <div class="left-header-content">
                                                        <h5>Project Summary</h5>
                                                        <p class="m-0 c-o-light">
                                                            Due date exceeded for 24 projects
                                                        </p>
                                                    </div>
                                                    <div class="card-header-right-btn">
                                                        <a class="c-o-light" href="project-list.html">View All</a>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="card-body pt-0">
                                                <div class="row g-3">

                                                    <!-- LEFT CONTENT -->
                                                    <div class="col-lg-6">
                                                        <div class="h-100">
                                                            <ul class="summary-section">
                                                                <li class="p-b-20">
                                                                    <p>
                                                                        The proposal's project summary is among its most
                                                                        crucial sections...
                                                                    </p>
                                                                </li>

                                                                <div class="row text-center mt-3">
                                                                    <div class="col-4">
                                                                        <div class="card-header border-t-primary">
                                                                            <h5>Total Data</h5>
                                                                            <p class="mt-1 f-m-light">Use class of
                                                                                <code>border-t-* </code> for the top border.
                                                                            </p>
                                                                            <h3>{{ count($checks) }}</h3>

                                                                        </div>
                                                                    </div>

                                                                    <div class="col-4">
                                                                        <div class="card-header border-t-danger">
                                                                            <h5>Rata-rata IMT</h5>
                                                                            <p class="mt-1 f-m-light">Use class of
                                                                                <code>border-t-* </code> for the top border.
                                                                            </p>
                                                                            <h3>{{ round($checks->avg('imt.imt'), 2) }}</h3>

                                                                        </div>
                                                                    </div>

                                                                    <div class="col-4">
                                                                        <div class="card-header border-t-danger">
                                                                            <h5>Rata-rata VO2</h5>
                                                                            <p class="mt-1 f-m-light">Use class of
                                                                                <code>border-t-* </code> for the top border.
                                                                            </p>
                                                                            <h3>{{ round($checks->avg('fitness.vo2_max'), 2) }}
                                                                            </h3>
                                                                        </div>
                                                                    </div>
                                                                </div>

                                                                <!-- Info bawah -->
                                                                <li>
                                                                    <ul class="common-space p-t-10">
                                                                        <li>
                                                                            <p class="mb-1">Creation Date</p>
                                                                            <span>14 March, 2024</span>

                                                                            <p class="mb-1 mt-2">Due Date</p>
                                                                            <span>30 April, 2024</span>
                                                                        </li>

                                                                        <li>
                                                                            <p class="mb-1">Priority</p>
                                                                            <span
                                                                                class="badge badge-light-primary">High</span>

                                                                            <p class="mb-1 mt-2">Status</p>
                                                                            <span class="badge badge-light-success">In
                                                                                progress</span>
                                                                        </li>

                                                                        <li>
                                                                            <p class="p-t-10 mb-2">Resource</p>
                                                                            <div class="attachment-file common-flex">
                                                                                <div class="common-flex align-items-center">
                                                                                    <img class="img-fluid"
                                                                                        src="../assets/images/project/files/pdf.png" />
                                                                                    <div>
                                                                                        <p class="mb-0">Projects Webflow
                                                                                        </p>
                                                                                        <p class="c-o-light">678 KB</p>
                                                                                    </div>
                                                                                </div>
                                                                                <a href="#" download>
                                                                                    <i
                                                                                        class="fa-solid fa-download f-light"></i>
                                                                                </a>
                                                                            </div>
                                                                        </li>
                                                                    </ul>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </div>

                                                    <!-- RIGHT CHARTS -->
                                                    <div class="col-lg-6">
                                                        <div class="row g-3">

                                                            <div class="col-md-6">
                                                                <div class="card p-3 h-100 card-header border-t-danger">
                                                                    <h5>Rata-rata VO2 Max</h5>
                                                                    <div style="height:250px;">
                                                                        <canvas id="vo2Chart"></canvas>
                                                                    </div>
                                                                </div>
                                                            </div>

                                                            <div class="col-md-6">
                                                                <div class="card p-3 h-100 card-header border-t-danger">
                                                                    <h5>Persentase Data Kategori Kebugaran</h5>
                                                                    <div style="height:250px;">
                                                                        <canvas id="imtChart"></canvas>
                                                                    </div>
                                                                </div>
                                                            </div>

                                                        </div>
                                                    </div>

                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-12 xl-100 box-col-12">
                                        <div class="card">
                                            <div class="card-header card-no-border">
                                                <div class="common-space">
                                                    <div class="left-header-content">
                                                        <h5>Projects Pending</h5>
                                                        <p class="m-0 c-o-light">
                                                            Total 28 projects pending
                                                        </p>
                                                    </div>
                                                    <div class="card-header-right-btn">
                                                        <a class="c-o-light" href="project-list.html">View All</a>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="card-body px-3 pt-2">
                                                <div
                                                    class="recent-table table-responsive currency-table recent-order-table custom-scrollbar">
                                                    <table class="table table-striped" id="main-recent-order">
                                                        <thead>
                                                            <tr>
                                                                <th>No</th>
                                                                <th>Tanggal</th>
                                                                <th>Nama</th>
                                                                <th>Unit</th>
                                                                <th>IMT</th>
                                                                <th>Kategori</th>
                                                                <th>VO2</th>
                                                                <th>Hasil</th>
                                                                <th>Create Date</th>
                                                                <th>Aksi</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            @foreach ($checks as $c)
                                                                <tr>
                                                                    <td>{{ $loop->iteration }}</td>
                                                                    <td>{{ $c->tanggal_pemeriksaan }}</td>
                                                                    <td>{{ $c->user->nama }}</td>
                                                                    <td>{{ $c->user->employee->unit->nama_unit ?? '-' }}
                                                                    </td>
                                                                    <td>{{ round($c->imt->imt ?? 0, 2) }}</td>
                                                                    <td>
                                                                        @php
                                                                            $kategori = strtolower(
                                                                                $c->imt->kategori_imt ?? '',
                                                                            );

                                                                            $badge = match (true) {
                                                                                str_contains($kategori, 'obesitas')
                                                                                    => 'bg-dark',
                                                                                str_contains($kategori, 'gemuk')
                                                                                    => 'bg-danger',
                                                                                str_contains($kategori, 'normal')
                                                                                    => 'bg-success',
                                                                                default => 'bg-secondary',
                                                                            };
                                                                        @endphp

                                                                        <span class="badge {{ $badge }}">
                                                                            {{ $c->imt->kategori_imt ?? '-' }}
                                                                        </span>
                                                                    </td>
                                                                    <td>
                                                                        {{ $c->fitness && $c->fitness->vo2_max ? round($c->fitness->vo2_max, 2) : '-' }}
                                                                    </td>
                                                                    <td>{{ $c->fitness->hasil ?? '-' }}</td>
                                                                    <td>{{ $c->created_at->format('d M Y') }}</td>
                                                                    <td>
                                                                        <button class="btn btn-info btn-sm btn-detail"
                                                                            data-bs-toggle="modal"
                                                                            data-bs-target="#detailModal{{ $c->id }}"
                                                                            data-nama="{{ $c->user->nama ?? '-' }}"
                                                                            data-unit="{{ $c->user->employee->unit->nama_unit ?? '-' }}"
                                                                            data-tanggal="{{ $c->tanggal_pemeriksaan }}"
                                                                            data-umur="{{ $c->umur }}"
                                                                            data-imt="{{ round($c->imt->imt ?? 0, 2) }}"
                                                                            data-kategori="{{ $c->imt->kategori_imt ?? '-' }}"
                                                                            data-vo2="{{ round($c->fitness->vo2_max ?? 0, 2) }}"
                                                                            data-hasil="{{ $c->fitness->hasil ?? '-' }}"
                                                                            data-frekuensi="{{ $c->exercise->frekuensi ?? '-' }}"
                                                                            data-durasi="{{ $c->exercise->durasi ?? '-' }}"
                                                                            data-tipe="{{ $c->exercise->tipe_latihan ?? '-' }}"
                                                                            data-denyut="{{ $c->exercise->denyut_nadi ?? '-' }}">
                                                                            Detail
                                                                        </button>
                                                                        @if ($c->exercise()->exists())
                                                                            <span class="badge bg-success">Sudah Ada</span>
                                                                        @else
                                                                            <button class="btn btn-primary btn-sm"
                                                                                data-bs-toggle="modal"
                                                                                data-bs-target="#modalExercise{{ $c->id }}">
                                                                                + Program Latihan
                                                                            </button>
                                                                        @endif
                                                                    </td>
                                                                </tr>
                                                                @include('health.modal-detail')
                                                                @include('health.modal_exercise')
                                                            @endforeach
                                                        </tbody>
                                                    </table>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
@push('scripts')
    <script>
        // IMT Chart
        if (document.getElementById('imtChart')) {
            const imtData = @json($imtStats ?? []);

            new Chart(document.getElementById('imtChart'), {
                type: 'doughnut',
                data: {
                    labels: Object.keys(imtData),
                    datasets: [{
                        data: Object.values(imtData)
                    }]
                }
            });
        }

        // VO2 Chart
        if (document.getElementById('vo2Chart')) {
            const vo2Data = @json($vo2Stats ?? []);

            new Chart(document.getElementById('vo2Chart'), {
                type: 'bar',
                data: {
                    labels: Object.keys(vo2Data),
                    datasets: [{
                        label: 'Rata-rata VO2 Max',
                        data: Object.values(vo2Data)
                    }]
                }
            });
        }
    </script>
@endpush
